...
const DUMMY_DATA: User.Search.Schema[] = [
  {
    data: {
      first_name: "Ying",
      last_name: "Yang",
      ...
      emails: ["yy2364@cornell.edu", "yangy2@myumanitoba.ca"],
      keywords: ["Cavity Magnonics, Spin defects, Nanomagnetism"],
      contact_address: [],
    },
    level_two_data: {
      other_publications: [],
  ...
  },
  {
    ...
    level_two_data: {
      ...
      grant_research: [
        ...
        {
          grant_title:
            "Characterization of novel mechanisms for inhibition of islet cell apoptosis",
        ...
        {
          grant_title:
            "Evaluation of islet xenograft survival and function using a novel bioengineered serum ultrafiltrate perfusion device in nonhuman primates",
          grant_agency: "Juvenile Diabetes Research Foundation",
      ...
      journal_publications: [
        ...
        {
          journal_publication_title:
            "MicroRNA-144 is unlikely to play a role in bronchiolitis obliterans syndrome: To the Editor",
          journal_publication_journal_name:
...
interface ISearchApiResponse {
  query: string;
  results: User.Search.Schema[];
}

type TSearch = (keyword: string) => Promise<ISearchApiResponse>;

const search: TSearch = async (keyword: string) => {
  console.log("Keyword", keyword);

  return { query: keyword, results: DUMMY_DATA };
};
